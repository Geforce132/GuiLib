...
public abstract class TextField extends FocusableWidget {
	...
	protected void drawCursorVertical(int x1, int y1, int x2, int y2) {
        ...
        bufferbuilder.pos((double) x1, (double) y2, 0.0D);
        bufferbuilder.pos((double) x2, (double) y2, 0.0D);
        bufferbuilder.pos((double) x2, (double) y1, 0.0D);
        bufferbuilder.pos((double) x1, (double) y1, 0.0D);
		tessellator.draw();
